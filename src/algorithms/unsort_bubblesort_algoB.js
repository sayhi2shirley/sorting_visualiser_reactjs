...
const bubbleSortAlgorithmB = (arr, ind, position, arrSteps, lastSortStep, arrColors) => {
    console.log(arrColors.length);
    
    let clrCode = arrColors[arrColors.length - 1].slice();
    
    bubbleSortAlgoB(arr, ind, arrSteps, lastSortStep, arrColors, clrCode, 1);
    ...
    bubbleSortAlgoB(ind, arr, arrSteps, lastSortStep, arrColors, clrCode, 0);
    console.log('UnSort ' + arrSteps.length + 'last step' + lastSortStep.length);
};

// Creating the bubbleSort function
const bubbleSortAlgoB = (arr, ind, stps, lastSortStep, colors, clrCode, sorting) => {
